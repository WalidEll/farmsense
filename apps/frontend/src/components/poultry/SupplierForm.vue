    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
        <h3 class="text-xl font-bold text-gray-900">
          {{ initialData ? t('common.edit') : t('suppliers_create') }}
        </h3>
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4 overflow-y-auto flex-1">
        <div>
          <label class="block text-sm font-bold text-gray-700 mb-1">
            {{ t('supplier_name') }} <span class="text-red-500">*</span>
          </label>
          <input
            v-model="form.name"
            type="text"
        ...
        <div>
          ...
          <input
            v-model="form.productsSupplied"
            type="text"
            class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-green-500 outline-none"
            :placeholder="t('supplier_products')"
          />
        </div>
...
const emit = defineEmits<{
  close: []
  submit: [data: CreateSupplierRequest]
}>()
...
const form = reactive<CreateSupplierRequest>({
  ...
  productsSupplied: props.initialData?.productsSupplied || '',
