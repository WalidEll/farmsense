# FarmSense — Claude Context

## Project Overview

FarmSense is a smart plant monitoring platform targeting Moroccan home plant owners (v0) and small farms/greenhouses (v1). It combines IoT sensors (ESP32), a REST API (Spring Boot), and a PWA (Vue 3) to monitor plants and send real-time WhatsApp alerts in Darija, Arabic, and French. AI-powered plant diagnosis uses the Claude Vision API.
...
- Spring WebFlux for async calls (WhatsApp + Claude APIs)
...
| Async I/O | Spring WebFlux used only for WhatsApp + Claude API calls |
...
| AI diagnosis | `DiagnoseService` calls Claude Vision API; results stored + retrievable |
